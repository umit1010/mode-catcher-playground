...
# ---- REGISTER DASH PAGE
dash.register_page(__name__, path='/', title=f"playground | mode-catcher | dev build ({datetime.today()})")
        ...
        if in_sentences:
            for sentence in doc.sents:
                ...
                if use_nlp_tags and first_parse:
                    excluded_in_row.extend(
                        [t.lemma_ for t in sentence if nlf.has_excluded_nlp_tag(t) and not t.is_stop])
                                        ...
                                        html.Div(
                                            [
                                                ...
                                                html.P([
                                                    ...
                                                    html.Br(),
                                                    html.Em(
                                                        f"\"{code_definitions[category][code]['verbatim_excerpt']}\"")
                                                ], className="ms-2",
                                                ),
                                            ],
    ...
    buttons_for_text = html.Div([
        html.Span(
            dbc.Button(
                ...
                color="light" if stops[i] else "danger" if lemmas[i] in tokens_excluded_from_lines.get(row,
                                                                                                       []) else "success",
                class_name="m-2",
                size="sm",
            )
    ...
    if len(connected_nodes) > 0:
        ...
        fig_metrics.update_layout(
            ...
            xaxis=dict(
                ...
                ticklabelstep=1 if len(degree_labels) < 50 else math.ceil(len(degree_labels) / 50),
                # shows all the tick labels if there are very few
...
if fs.SAMPLES_FOLDER.is_dir():
    # gets all txt files
    text_files = [f.name for f in sorted(fs.SAMPLES_FOLDER.glob("*.txt"))]
    if len(text_files) > 0:
            ...
            [
                dbc.Col(
                    dbc.InputGroup([
                        ...
                        dbc.Input(id="min-co", type="number", min=1, max=10, step=1, value=1, persistence=True,
                                  debounce=300),
                    ]),
                    class_name="mt-2",
                    md=6, lg=4, xl=3,
                ...
                dbc.Col(
                    dbc.InputGroup([
                        ...
                        dbc.Input(id="min-strong-co", type="number", min=1, max=10, step=1, value=2, persistence=True,
                                  debounce=300)
                    ]),
                    class_name="mt-2",
                    md=6, lg=4, xl=3,
            ...
            [
                dbc.Col(
                    dbc.InputGroup([
                        ...
                        dbc.Input(id="node-size", type="number", min=1, max=40, step=1, value=5, persistence=True,
                                  debounce=500),
                    ]),
                    lg=3,
                    xl=2,
                ...
                dbc.Col(
                    dbc.InputGroup([
                        ...
                        dbc.Input(id="layout-iterations", type="number", min=0, max=500, step=1, value=10,
                                  persistence=True, debounce=500),
                    ]),
                    lg=4,
                    xl=3,
                ),
...
coding_modal = dbc.Modal(
    [
        ...
        dbc.ModalFooter([
            ...
            dbc.Button("excluded only for this line", id="exclude-key-button", color="danger", class_name="m-1",
                       size="sm", ),
            ...
            dbc.Tooltip("Gray tokens are excluded from analysis for the entire transcript.",
                        target="stopword-key-button", placement="right"),
            dbc.Tooltip(
                "Red tokens are excluded from analysis only for this line but may be included in the other lines.",
                target="exclude-key-button", placement="right"),
...
def load_input_file_callback(file_name, is_sentencized, spacy_model):
    ...
    file_path = fs.SAMPLES_FOLDER / file_name
...
def parse_button_callback(
    ...
    cached_parsed_data, deductive_codes, code_definitions, stopped_tokens, unstopped_tokens = unpickle_defaults_and_model(
        mode_name, spacy_model, sentencized)
    ...
    if filename == "__manual entry__":
        input_file = fs.SAMPLES_FOLDER / f'{mode_name}.txt'
    else:
        input_file = fs.SAMPLES_FOLDER / filename

    backup_file = fs.SAMPLES_FOLDER / f'{filename}backup'
